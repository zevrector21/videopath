...
class TotalAnalyticsDataViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self, vid = None, *args, **kwargs):
        ...
        vid = m.group(1)
        return TotalAnalyticsData.objects.filter(video__user=self.request.user, video__id=vid)
...
class DailyAnalyticsDataViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self, vid=None):
        ...
        return DailyAnalyticsData.objects.filter(video__user=self.request.user, video__id=vid, date__range=[start, end])
