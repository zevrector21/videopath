...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'OneTimeAuthenticationToken.id'
        db.delete_column(u'users_onetimeauthenticationtoken', u'id')

        # Adding field 'OneTimeAuthenticationToken.key'
        db.add_column(u'users_onetimeauthenticationtoken', 'key',
                      self.gf('django.db.models.fields.CharField')(
                          default='abc', max_length=40, primary_key=True),
                      keep_default=False)

    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for
        # 'OneTimeAuthenticationToken.id'
        raise RuntimeError(
            "Cannot reverse this migration. 'OneTimeAuthenticationToken.id' and its values cannot be restored.")

        # The following code is provided here to aid in writing a correct
        # migration        # Adding field 'OneTimeAuthenticationToken.id'
        db.add_column(u'users_onetimeauthenticationtoken', u'id',
                      self.gf('django.db.models.fields.AutoField')(
                          primary_key=True),
                      keep_default=False)

        # Deleting field 'OneTimeAuthenticationToken.key'
        db.delete_column(u'users_onetimeauthenticationtoken', 'key')
